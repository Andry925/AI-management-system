...
async def test_create_note(async_client):
    response = await async_client.post("/api/v1/notes/new-note", json=NOTE_DATA)
...
async def test_get_my_notes(async_client, create_random_note):
    response = await async_client.post("/api/v1/notes/new-note", json=NEW_NOTE)
    assert response.status_code == 201
    get_response = await async_client.get("/api/v1/notes/my-notes/1/10")
...
async def test_update_note(async_client, create_random_note):
    ...
    note_id = note.id
    update_resp = await async_client.put(f"/api/v1/notes/note-update/{note_id}", json=UPDATED_NOTE_DATA)
...
async def test_update_non_existent_note(async_client, create_random_note):
    invalid_id = 343432
    update_fail = await async_client.put(f"/api/v1/notes/note-update/{invalid_id}", json=UPDATED_NOTE_DATA)
...
async def test_delete_note(async_client, create_random_note):
    note = create_random_note
    note_id = note.id
    response = await async_client.delete(f"/api/v1/notes/delete-note/{note_id}")
    assert response.status_code == 204
    response_after_deletion = await async_client.get(f"/api/v1/notes/single-note/{note_id}")
...
async def test_get_single_note(async_client, create_random_note):
    note = create_random_note
    note_id = note.id
    response = await async_client.get(f"/api/v1/notes/single-note/{note_id}")
...
async def test_get_note_history(async_client, create_random_note):
    ...
    response = await async_client.put(f"/api/v1/notes/note-update/{note_id}", json=update_request)
    assert response.status_code == 200
    history_resp = await async_client.get(f"/api/v1/notes/note/history/{note_id}/1/10")
