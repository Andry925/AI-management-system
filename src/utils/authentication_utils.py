...
def get_current_user(refresh_token: Annotated[str, Depends(oauth2_bearer)]) -> dict:
...
def decode_refresh_token(refresh_token: str) -> dict:
    try:
        ...
        raise HTTPException(status_code=status.HTTP_401_UNAUTHORIZED, detail=f'Refresh token is invalid: {str(e)}')
